...
function sdrFreqExportSheet(sheet) {
    ...
    for (var i = 2; i < values.length; i++) {
        doc.push('<preset id="' + id + '" ' +
            'name="' + values[i][4] + '" ' +
            'freq="' + rawFreq(values[i][2]) + '" ' +
            'centfreq="' + rawFreq(values[i][2]) + '" ' +
            'offset="0" ' +
            'order="' + (i - 1) + '" ' +
            'filter="' + rawFilter(values[i][3], values[i][1]) + '" ' +
            'dem="' + rawDim(values[i][1]) + '"/>');
        id++
    }
    doc.push('</category>');
    return doc.join("\n");
...
function rawFilter(filter, type) {
    var dicFilter = {
        FM: 105000,
        NFM: 70000,
        AM: 70000,
